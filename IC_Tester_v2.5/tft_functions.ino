/*
 *  tft_functions contains all display related routines
 *  
...
void startScreen()
{
  ...
  printText( 25, 35, 5, TTGREEN, "IC TESTER");
  printText(150, 90, 3, WHITE, "By");
  printText( 35,120, 3, WHITE, "Frank Hellmann");
  printText( 20,220, 2, CYAN,  "Touch anywhere to START");
}
...
void drawStatus(String text, int col)
{
  ...
  int len = text.length();
  if (len > 17) len=17;
  tft.fillRect(100,218,319,239,BLACK);
...
void drawIC(int numPins, String number)
{
  ...
  // Clear
  tft.fillRect(sx-80,sy-6,230,192,BLACK);

  // Draw IC box
  tft.fillCircle(sx+30,sy+1,10,GRAY);
  tft.fillRect(sx+20,sy-10,21,10,BLACK);
  tft.drawFastHLine(0,19,319,GRAY);
  tft.drawRect(sx,sy,60,184,TTGREEN);
  tft.drawRect(sx+1,sy+1,58,128+((numPins-14)*9),WHITE);
  
  for (int i = 0; i < (numPins/2); i++) {
    // draw Pin boxes
    tft.drawRect(sx-15,sy+4+(i*18),14,14,WHITE);
    tft.drawRect(sx+60+1,sy+4+(i*18),14,14,WHITE);
    ...
    num = num + String(1+i);
    printText(sx-40,sy+4+(i*18),2,GRAY,num);
    num = String(numPins-i);
    printText(sx+60+17,sy+4+(i*18),2,GRAY,num);
    ...
    if (number!=""){
      ...
      printText(sx-52-(len*5),sy+4+(i*18),2,col,pinn);
      ...
      printText(sx+128-(len*5),sy+4+(i*18),2,col,pinn);

      // Paint Error Indicator Boxes
      if (errpin[i]) {
        tft.fillRect(sx-14,sy+5+(i*18),12,11,RED);
        errors=true;
      }
      if (errpin[numPins-i-1]) {
        tft.fillRect(sx+62,sy+5+(i*18),12,11,RED);
        errors=true;
      }
    }
  }
  ...
  if (number!=""){
    ...
    if (errors) {  // BAD IC = RED Number
      printText(sy+30,sx-52,3,RED,number);
    } else {
      printText(sy+30,sx-52,3,WHITE,number);
    }
    tft.setRotation(1);
  }
}
...
void modeScreen()
{
  ...
  buttons[0].initButton(&tft, 45,  50, 80, 50, GRAY, TTGREEN, WHITE, (char *)"Auto", 2);
  buttons[0].drawButton();

  buttons[1].initButton(&tft, 45, 105, 80, 50, GRAY, TTGREEN, WHITE, (char *)"Manual", 2);
  buttons[1].drawButton();

  buttons[2].initButton(&tft, 45, 160, 80, 50, GRAY, TTGREEN, WHITE, (char *)"Pins", 2);
